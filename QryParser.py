...
class QryParser:
    ...
    def __createOperator(operatorName):
        ...
        if operatorNameLowerCase == '#or':
            ...
            _, n = operatorNameLowerCase.split('/')
            operator = QryIopNear(int(n))
        else:
    ...
    def parseString(queryString):
        ...
        weighted = isinstance(queryTree, QrySopWSum)
        weights = []
	    ...
	    # STUDENTS:
            ...
            qargs = []
            if weighted:
                weight = float(queryString.split()[0])
                queryString = ' '.join(queryString.split()[1:])
